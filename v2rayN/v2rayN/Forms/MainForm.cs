...
namespace v2rayN.Forms
{
    public partial class MainForm : BaseForm
    {
        ...
        private void InitServersView()
        {
            ...
            lvwColumnSorter = new ListViewColumnSorter(9, SortOrder.Descending);
            this.lvServers.ListViewItemSorter = lvwColumnSorter;
            lvServers.EndUpdate();
        }
        ...
        private void Speedtest(string actionType)
        {
            ...
            ClearTestResult();
            SpeedtestHandler statistics = new SpeedtestHandler(ref config, ref v2rayHandler, lvSelecteds, actionType, UpdateSpeedtestHandler,RemoveUselessServer,SelectFasterServer);
        ...
        #region 删除无用服务器
        private void RemoveUselessServer()
        {
            lvServers.Invoke((Action)delegate
            {
                var list = new List<int>();

                for (int i = 0; i < lvServers.Items.Count; i++)
                {
                    var textResult = lvServers.Items[i].SubItems["testResult"].Text;
                    if (!textResult.Contains("M/s") && !textResult.Contains("ms") && !textResult.IsNullOrWhiteSpace())
                    {
                        list.Add(i);
                    }
                }

                ConfigHandler.RemoveServer(ref config, list);
                RefreshServers();
                LoadV2ray();
            });
        ...
        private void SelectFasterServer()
        {
            lvServers.Invoke((Action) delegate
            {
                try
                {
                    ...
                    //{
                    //    var textResult = server.SubItems["testResult"].Text;
                    //    if (textResult.Contains("M/s"))
                    //    {
                    //        var value = float.Parse(textResult.Replace("M/s", ""));
                    //        speedList.Add(server.Index, value);
                    //    }
                    //}

                    //var maxSpeed = speedList.Max((x) => x.Value);
                    //var index = speedList.FirstOrDefault(x => x.Value == maxSpeed).Key;
                    

                    //使服务器列表倒序，选中第一个最快节点
                    var column = 9;
                    bool asc = false;
                    if (ConfigHandler.SortServers(ref config, (EServerColName)column, asc) != 0)
                    {
                        return;
                    }
                    lvServers.Columns[9].Tag = Convert.ToString(asc);
                    RefreshServers();
                    SetDefaultServer(0);
                    //RefreshServers();
                    //LoadV2ray();
